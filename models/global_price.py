...
class GlobalPrice:
    ...
    def validate(self):
        ...
        if self.website_id:
            ...
            if not website:
                errors.append(f"Website with ID {self.website_id} does not exist")

        # Validate format exists if specified
        if self.format_id:
            format = AdsFormat.get_by_id(self.format_id)
            if not format:
                errors.append(f"Format with ID {self.format_id} does not exist")
        ...
        if self.uniform_buy_price_unit_id:
            ...
            if not price_type:
                errors.append(f"Price type with ID {self.uniform_buy_price_unit_id} does not exist")
        ...
        if self.uniform_sell_price_unit_id:
            ...
            if not price_type:
                errors.append(f"Price type with ID {self.uniform_sell_price_unit_id} does not exist")
            price_types.append(price_type)

        if self.buy_price_unit_id:
            price_type = PriceType.get_by_id(self.buy_price_unit_id)
            if not price_type:
                errors.append(f"Price type with ID {self.buy_price_unit_id} does not exist")
            price_types.append(price_type)

        if self.sell_price_unit_id:
            price_type = PriceType.get_by_id(self.sell_price_unit_id)
            if not price_type:
                errors.append(f"Price type with ID {self.sell_price_unit_id} does not exist")
            price_types.append(price_type)
        ...
        if self.format_id and not self.global_price_id:  # Only for new records
            existing = self.get_active_by_website_format(self.website_id, self.format_id)
            if existing:
                errors.append(f"Active price already exists for website {self.website_id} and format {self.format_id}")
            ...
            with get_db_connection() as conn:
                ...
                cursor.execute('''
                    SELECT gp.*, w.domain_website, p.email,
                           pt1.price_type_name as uniform_buy_price_type_name,
                           pt2.price_type_name as uniform_sell_price_type_name,
                           pt3.price_type_name as buy_price_type_name,
                           pt4.price_type_name as sell_price_type_name,
                           af.format_name,
                    ...
                    LEFT JOIN price_type pt5 ON gp.uniform_buy_price_unit_id = pt5.price_type_id
                    LEFT JOIN price_type pt6 ON gp.uniform_sell_price_unit_id = pt6.price_type_id
                    LEFT JOIN price_type pt7 ON gp.buy_price_unit_id = pt7.price_type_id
                    LEFT JOIN price_type pt8 ON gp.sell_price_unit_id = pt8.price_type_id
